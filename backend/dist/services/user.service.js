...
class UserService {
    ...
    async getUserById(id) {
        ...
        if (!user) {
            throw new errors_1.ApiError(404, "USER_NOT_FOUND", `Користувача з ID ${id} не знайдено`);
        }
    ...
    async createUser(dto) {
        ...
        if (validationErrors.length > 0) {
            throw new errors_1.ApiError(400, "VALIDATION_ERROR", "Помилка валідації полів користувача", validationErrors);
        }
        const newUser = {
    ...
    async updateUser(id, dto, isPartial = false) {
        const existingUser = await this.userRepository.findById(id);
        if (!existingUser) {
            throw new errors_1.ApiError(404, "USER_NOT_FOUND", `Користувача з ID ${id} не знайдено для оновлення`);
        }
        const validationErrors = [];
        ...
        if (dto.email && dto.email.trim() !== "") {
            const userWithEmail = await this.userRepository.findByEmail(dto.email.trim());
        ...
        if (validationErrors.length > 0) {
            throw new errors_1.ApiError(400, "VALIDATION_ERROR", "Помилка валідації при оновленні користувача", validationErrors);
        }
        ...
        if (dto.email !== undefined)
            fieldsToUpdate.email = dto.email.trim();
        if (dto.password !== undefined)
            fieldsToUpdate.password = dto.password;
        if (dto.role !== undefined)
            fieldsToUpdate.role = dto.role;
        const updatedUser = await this.userRepository.update(id, fieldsToUpdate);
        if (!updatedUser) {
            throw new errors_1.ApiError(500, "INTERNAL_SERVER_ERROR", "Не вдалося оновити користувача");
        }
    ...
    async deleteUser(id) {
        ...
        if (!deleted) {
            throw new errors_1.ApiError(404, "USER_NOT_FOUND", `Користувача з ID ${id} не знайдено для видалення`);
        }
    }
}
