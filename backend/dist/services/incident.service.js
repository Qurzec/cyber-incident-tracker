...
class IncidentService {
    ...
    async getIncidentById(id) {
        ...
        if (!incident) {
            throw new errors_1.ApiError(404, "INCIDENT_NOT_FOUND", `Інцидент з ID ${id} не знайдено`);
        }
    ...
    async createIncident(dto, currentUserId) {
        ...
        if (validationErrors.length > 0) {
            throw new errors_1.ApiError(400, "VALIDATION_ERROR", "Помилка валідації полів інциденту", validationErrors);
        }
        const newIncident = {
            date: dto.date.trim(),
            tag: dto.tag.trim(),
    ...
    async updateIncident(id, dto, isPartial = false, currentUserId) {
        ...
        if (validationErrors.length > 0) {
            throw new errors_1.ApiError(400, "VALIDATION_ERROR", "Помилка валідації при оновленні інциденту", validationErrors);
        }
        // Зливаємо поля відповідно до типу оновлення
        const fieldsToUpdate = {};
        if (dto.date !== undefined)
            fieldsToUpdate.date = dto.date.trim();
        if (dto.tag !== undefined)
            fieldsToUpdate.tag = dto.tag.trim();
        ...
        const updatedIncident = await this.incidentRepository.update(id, fieldsToUpdate);
        if (!updatedIncident) {
            throw new errors_1.ApiError(500, "INTERNAL_SERVER_ERROR", "Не вдалося оновити інцидент");
        }
    ...
    async addComment(incidentId, userId, message) {
        ...
        if (!message || message.trim() === "") {
            throw new errors_1.ApiError(400, "VALIDATION_ERROR", "Текст коментаря не може бути порожнім");
        }
        return await this.incidentRepository.addComment(incidentId, userId, message);
    }
